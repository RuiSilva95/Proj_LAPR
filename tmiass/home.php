    ...
    <form id="filter" name="filter" method="POST" action="<?php echo $current_file; ?>">
      ...
      <table align="center" name="list" id="list">
        ...
        <?php
            ...
            //*************************************************************
			$campos_query = "`client`.`name` AS 'client_name' ,`equipment`.`entity` AS 'enty' , `equip_status`.`status` AS 'stats' , `equipment`.`id_client` AS 'id_cli' , `users`.`name` AS 'user_name' ,`equipment`.`mark/model` AS 'mark' , `equipment`.`id` AS 'idd'";
