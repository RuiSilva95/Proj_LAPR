...
<div id="content">
  ...
  <div class="bodycontent">
	...
	if(isset($_POST['submit'])){
		...
		mysql_query("UPDATE `users` SET `name`='".$name."' WHERE `id_user`='".access('id_user')."'")or die(mysql_error());
	}
	?>
    <table width="305" >
      <tr>
			...
			if(md5($old_password) == access('password')){
				if(!empty($new_password) && !empty($again_password)){
					if($new_password == $again_password){
						mysql_query("UPDATE `users` SET `password`='".md5($new_password)."' WHERE `id_user`='".access('id_user')."'")or die(mysql_error());
